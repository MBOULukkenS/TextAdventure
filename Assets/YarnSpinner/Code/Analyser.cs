...
namespace Yarn.Analysis
{
    ...
    public class Context {
        ...
        public Context ()
        {
            ...
            foreach (Type analyserType in defaultAnalyserClasses) {
        ...
        public Context(params Type[] types) {
            ...
            foreach (Type analyserType in types) {
                analysers.Add((CompiledProgramAnalyser)Activator.CreateInstance (analyserType));
            }
        }
    ...
    internal class VariableLister : CompiledProgramAnalyser {
        ...
        public override void Diagnose(Program program)
        {
            ...
            foreach (KeyValuePair<string, Node> nodeInfo in program.nodes)
            {

                string nodeName = nodeInfo.Key;
                Node theNode = nodeInfo.Value;

                foreach (Instruction instruction in theNode.instructions)
                {
        ...
        {
            return variables
                .Select(variable => new Diagnosis("Script uses variable " + variable, Diagnosis.Severity.Note))
                .ToList();
        }
    }
    ...
    internal class UnusedVariableChecker : CompiledProgramAnalyser {
        ...
        public override IEnumerable<Diagnosis> GatherDiagnoses ()
        {
            ...
            HashSet<string> readOnlyVariables = new HashSet<string> (readVariables);
            readOnlyVariables.ExceptWith (writtenVariables);

            // Exclude written variables that are also read
            HashSet<string> writeOnlyVariables = new HashSet<string> (writtenVariables);
            writeOnlyVariables.ExceptWith (readVariables);

            // Generate diagnoses
            List<Diagnosis> diagnoses = new List<Diagnosis>();

            foreach (string readOnlyVariable in readOnlyVariables) {
                string message = string.Format (CultureInfo.CurrentCulture, "Variable {0} is read from, but never assigned", readOnlyVariable);
                diagnoses.Add(new Diagnosis (message, Diagnosis.Severity.Warning));
            }

            foreach (string writeOnlyVariable in writeOnlyVariables) {
                string message = string.Format (CultureInfo.CurrentCulture, "Variable {0} is assigned, but never read from", writeOnlyVariable);
                diagnoses.Add(new Diagnosis (message, Diagnosis.Severity.Warning));
            }
