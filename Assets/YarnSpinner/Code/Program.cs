...
namespace Yarn
{
	...
	internal class Program
	{
		...
		{
			foreach (KeyValuePair<string, string> entry in newStrings)
			{
		...
		public string DumpCode(Library l)
		{
			...
			foreach (KeyValuePair<string, Node> entry in nodes)
			{
				...
				foreach (Instruction instruction in entry.Value.instructions)
				{
			...
			{
				LineInfo lineInfo = this.lineInfo[entry.Key];
		...
		{
			foreach (KeyValuePair<string, Node> otherNodeName in otherProgram.nodes)
			{
			...
			foreach (KeyValuePair<string, string> otherString in otherProgram.strings)
			{
	...
	struct Instruction
	{
		...
		public string ToString(Program p, Library l)
		{
			...
			string opAString = operandA != null ? Convert.ToString(operandA, CultureInfo.InvariantCulture) : "";
			string opBString = operandB != null ? Convert.ToString(operandB, CultureInfo.InvariantCulture) : "";
