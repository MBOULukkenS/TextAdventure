    ...
    internal class Compiler
    {
        ...
        internal void CompileNode(Parser.Node node) {
            ...
            if (node.source != null)
            {
                ...
                bool hasRemainingOptions = false;
                foreach (Instruction instruction in compiledNode.instructions)
                {
        ...
        void GenerateCode(Node node, Parser.Statement statement) {
            ...
            case Parser.Statement.Type.Block:
                ...
                foreach (Parser.Statement blockStatement in statement.block.statements) {
        ...
        void GenerateCode(Node node, Parser.ShortcutOptionGroup statement) {
            ...
            string endOfGroupLabel = RegisterLabel ("group_end");

            List<string> labels = new List<string> ();

            int optionCount = 0;
            foreach (Parser.ShortcutOption shortcutOption in statement.options) {

                string optionDestinationLabel = RegisterLabel ("option_" + (optionCount+1));
                ...
                string labelLineID = GetLineIDFromNodeTags(shortcutOption);

                string labelStringID = program.RegisterString (shortcutOption.label, node.name, labelLineID, shortcutOption.lineNumber, true);
        ...
        void GenerateCode(Node node, Parser.IfStatement statement) {
            ...
            string endOfIfStatementLabel = RegisterLabel ("endif");

            foreach (Parser.IfStatement.Clause clause in statement.clauses) {
                string endOfClauseLabel = RegisterLabel ("skipclause");
        ...
        void GenerateCode(Node node, Parser.OptionStatement statement) {
            ...
            if (statement.label == null) {
                ...
                string stringID = program.RegisterString (statement.label, node.name, lineID, statement.lineNumber, true);
        ...
        void GenerateCode(Node node, Parser.ValueNode value) {
            ...
            case Value.Type.String:
                ...
                string id = program.RegisterString (value.value.stringValue, node.name, null, value.lineNumber, false);
